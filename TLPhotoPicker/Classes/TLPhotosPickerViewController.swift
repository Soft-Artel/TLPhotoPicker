...
public protocol PhotoEditorDelegate: class{
    func openPhotoEditorWithCamera(parentVC:UIViewController, complition: ((Bool) -> ())?)
}
...
extension TLPhotosPickerViewController {
    ...
    private func initItemSize() {
        ...
        let widthScreen = sizeWidth < sizeHeight ? sizeWidth : sizeHeight
        let width = ((widthScreen-(5*(count-1)))/count).rounded(.down)
